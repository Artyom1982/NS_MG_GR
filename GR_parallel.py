...
def SOLUTION_OF_TOV(args):
    ...
    #левой границе внешней области (u_B) и правой границе внешней области (u_inf)
    ...
    for i in range (1000):
        ...
        data_x.append(F3*R1*r_g*10**(-5)/(1-x1))
        data_m.append(m)
        data_J.append(J_rot)
        data_N.append(F0)
        data_B.append(F1)
        ...
        data_A.append(F3)
        curv.append(0)
        G.write(str(R1*F1*r_g*10**(-5)/(1-x1)))
        G.write(' ')
        G.write(str(F0))
        G.write(' ')
        G.write(str(F1))
    ...
    sigma_omega_out = (4*u_out_3.dx(0)*u_out_1.dx(0)*(1/H_r_out)**2+4*((1/H_theta_out)**2)*u_out_3.dx(1)*u_out_1.dx(1))*factor_1_out\
    -(3*u_out_3.dx(0)*u_out_2.dx(0)*(1/H_r_out)**2+3*(1/H_theta_out)**2*u_out_3.dx(1)*u_out_2.dx(1))*factor_1_out\

    if Omega0==0:
        q_0 = (Mass_ADM+DM)**(-1)*assemble(sigma_nu*J1*dx)
        q_2 = 0
        q_2_out = 0
        b_0 = -0.25
        omega_0 = 0
    ...
    if Omega0!=0:
        ...
        b_0 = -0.25
        omega_0 = (1/2)*(Mass_ADM+DM)**(-2)*assemble(sigma_omega*factor_1*J1*dx)
        omega_0_out = (1/2)*(Mass_ADM+DM)**(-2)*assemble(sigma_omega_out*factor_1_out*J1_out*dx)
        omega_2 = (1/12)*(Mass_ADM+DM)**(-4)*assemble(sigma_omega*DP_3*factor_2*factor_1*J1*dx)

    M_2 = -(1/3)*(1+4*b_0+3*(q_2+0*q_2_out))*(Mass_ADM+DM)**(3)
    ...
    print("Производные на границе внешней области =", derivative_out)
    print("квадратичное отклонение =", delta)
    print("относительное квадратичное отклонение =", delta_rel)
    print("GRV2=",(GRV2_1+GRV2_2+GRV2_2_out+GRV2_3+GRV2_3_out)*2)
    ...
    print("b0 =", b_0)
    print("q0 =", q_0)
    print("omega0 =", omega_0, omega_0_out)
    print("q2 =", q_2)
